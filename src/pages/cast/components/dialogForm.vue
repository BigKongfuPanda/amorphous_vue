  ...
  <el-dialog
    :title="
      dialogData.formType === 'add'
        ? `新增喷带记录-${$route.params.castId}号机组`
        : `修改喷带记录-${$route.params.castId}号机组`
    "
      ...
      <el-row :gutter="20">
        <el-col :span="6">
          <el-form-item label="制带炉号" prop="furnace" class="dialog_field">
            <el-input
              v-model="formData.furnace"
              v-if="dialogData.formType === 'edit'"
            ></el-input>
            <el-select
              v-model="formData.furnace"
              placeholder="请选择"
        ...
        <el-col :span="6">
          <el-form-item label="规格" prop="ribbonWidth" class="dialog_field">
            <el-select v-model="formData.ribbonWidth" placeholder="请选择">
              <el-option
                v-for="item in dialogData.ribbonWidthList"
      ...
      <el-row :gutter="20">
        <el-col :span="6">
          <el-form-item label="在线包号" prop="tundish" class="dialog_field">
            <el-input
              v-model="formData.tundish"
              placeholder="请输入包体编号数字"
            ></el-input>
          </el-form-item>
        </el-col>
        <el-col :span="6">
      ...
      <el-row :gutter="20">
        <el-col :span="6">
          <el-form-item label="所在班组" prop="team">
            <el-select v-model="formData.team">
              <el-option
                :label="formData.castId + '#-甲'"
                :value="formData.castId + '#-甲'"
              ></el-option>
              <el-option
                :label="formData.castId + '#-乙'"
                :value="formData.castId + '#-乙'"
              ></el-option>
              <el-option
                :label="formData.castId + '#-丙'"
                :value="formData.castId + '#-丙'"
              ></el-option>
              <el-option
                :label="formData.castId + '#-丁'"
                :value="formData.castId + '#-丁'"
              ></el-option>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="开包次数" prop="castTimes">
            <el-select
              v-model="formData.castTimes"
              @change="castTimesChangeHandler"
            >
        ...
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item
              label="喷嘴规格"
              class="dialog_field"
              :prop="'record.' + index + '.nozzleSize'"
              :rules="[
                {
                  required: true,
                  message: '请填写喷嘴规格，格式 30*0.25',
                  trigger: 'blur'
                },
            ...
            >
              <el-input
                v-model="item.nozzleSize"
                placeholder="格式：30*0.25"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item
              label="车修铜辊"
              class="dialog_field"
              :prop="'record.' + index + '.treatCoolRoller'"
              :rules="[
          ...
          <el-col :span="6">
            <el-form-item
              label="铜套厚度"
              class="dialog_field"
              :prop="'record.' + index + '.coolRollerThickness'"
              :rules="[
                {
                  required: true,
                  message: '请填写铜套厚度，单位 mm',
                  trigger: 'blur'
                },
                {
                  pattern: /^(0|[1-9]\d{0,8})$/,
                  message: '请输入9位以内自然数, 如 0,1,2,3,...',
                  trigger: 'blur'
                }
              ]"
            >
          ...
          <el-col :span="6">
            <el-form-item
              label="接钢时间"
              class="dialog_field"
              :prop="'record.' + index + '.ReceiveMeltTime'"
              :rules="[
                {
                  required: true,
                  message: '请填写接钢时间，格式 08:30',
                  trigger: 'blur'
                },
                { max: 10, message: '最多10位字符', trigger: 'blur' }
              ]"
            >
              <el-input
                v-model="item.ReceiveMeltTime"
                placeholder="格式：08:30"
              ></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item
              label="接钢前包温"
              class="dialog_field"
              :prop="'record.' + index + '.tundishTemperatureWithoutMelt'"
              :rules="[
                {
                  required: true,
                  message: '请填写接钢前包温',
          ...
          <el-col :span="6">
            <el-form-item
              label="接钢后包温"
              class="dialog_field"
              :prop="'record.' + index + '.tundishTemperatureWithMelt'"
              :rules="[
                {
                  required: true,
                  message: '请填写接钢后包温',
          ...
          <el-col :span="6">
            <el-form-item
              label="装杯时间"
              class="dialog_field"
              :prop="'record.' + index + '.installNozzleTime'"
              :rules="[
                {
                  required: true,
                  message: '请填写装杯时间，格式 08:40',
                  trigger: 'blur'
                },
                { max: 10, message: '最多10位字符', trigger: 'blur' }
              ]"
            >
              <el-input
                v-model="item.installNozzleTime"
                placeholder="格式：08:40"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item
              label="开包时间"
              class="dialog_field"
              :prop="'record.' + index + '.castTimeStart'"
              :rules="[
                {
                  required: true,
                  message: '请填写开包时间，格式 08:40',
                  trigger: 'blur'
                },
                { max: 10, message: '最多10位字符', trigger: 'blur' }
              ]"
            >
              <el-input
                v-model="item.castTimeStart"
                placeholder="格式：09:20"
              ></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item
              label="开包压力"
              class="dialog_field"
              :prop="'record.' + index + '.pressure'"
              :rules="[
                { required: true, message: '请填写开包压力', trigger: 'blur' },
                {
                  pattern: /^[1-9]\d{0,8}$/,
                  message: '请输入9位以内正整数, 如 18,...',
                  trigger: 'blur'
                }
              ]"
            >
          ...
          <el-col :span="6">
            <el-form-item
              label="开包温度"
              class="dialog_field"
              :prop="'record.' + index + '.tundishTemperatureCasting'"
              :rules="[
          ...
          <el-col :span="6">
            <el-form-item
              label="喷带前水温"
              class="dialog_field"
              :prop="'record.' + index + '.coolRollerTemperatureBeforeCast'"
              :rules="[
                { required: true, message: '请填写水温', trigger: 'blur' },
                {
                  pattern: /^(\d+|\d+\.\d{1,2})-(\d+|\d+\.\d{1,2})$/,
                  message: '格式错误, 如 32.50-32.50',
                  trigger: 'blur'
                }
              ]"
            >
              <el-input
                v-model="item.coolRollerTemperatureBeforeCast"
                placeholder="格式：32-32"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
            <el-form-item
              label="喷带后水温"
              class="dialog_field"
              :prop="'record.' + index + '.coolRollerTemperatureAfterCast'"
              :rules="[
                { required: true, message: '请填写水温', trigger: 'blur' },
                {
                  pattern: /^(\d+|\d+\.\d{1,2})-(\d+|\d+\.\d{1,2})$/,
                  message: '格式错误, 如 32.50-33.80',
                  trigger: 'blur'
                }
              ]"
            >
              <el-input
                v-model="item.coolRollerTemperatureAfterCast"
                placeholder="格式：32-35"
              ></el-input>
            </el-form-item>
          </el-col>
        </el-row>
        <el-row :gutter="20">
          <el-col :span="6">
            <el-form-item
              label="喷带位置"
              class="dialog_field"
              :prop="'record.' + index + '.castLocation'"
              :rules="[
          ...
          <el-col :span="6">
            <el-form-item
              label="完成时间"
              class="dialog_field"
              :prop="'record.' + index + '.castTimeEnd'"
              :rules="[
                {
                  required: true,
                  message: '请填写完成时间，格式 08:40',
                  trigger: 'blur'
                },
                { max: 10, message: '最多10位字符', trigger: 'blur' }
              ]"
            >
              <el-input
                v-model="item.castTimeEnd"
                placeholder="格式：10:30"
              ></el-input>
            </el-form-item>
          </el-col>
          <el-col :span="6">
...
export default {
  data() {
    return {
      ...
      visible: false,
      loading: false,
      meltList: [], // 根据化钢记录中的生产计划筛选出，还未喷带的炉号
      formData: {
        castTimes: 1, //开包次数，默认为1
        castId: 6, // 机组编号
        team: "", //所在班组
      ...
      rules: {
        ...
        tundishCar: [
          { required: true, message: "请选择包车编号", trigger: "blur" }
        ],
        isChangeTundish: [
          { required: true, message: "请选择是否换包", trigger: "blur" }
        ],
        nozzleNum: [
        ...
        remark: [{ max: 50, message: "最多50位字符", trigger: "blur" }],
        castTimes: [
          { required: true, message: "请选择开包次数", trigger: "blur" }
        ],
        uselessRibbonWeight: [
  ...
  created() {
    ...
    if (this.dialogData.formType === "add") {
      ...
      this.formData = Object.assign({}, _formConfig, {
        castId: Number(this.$route.params.castId),
        caster: this.userinfo.adminname
      });
    } else {
  ...
  methods: {
    queryMeltData() {
      this.meltList = [];
      this.$http("get", urlmap.queryMeltDataByCast, {
        castId: this.formData.castId
        // startTime: "",
        // endTime: ""
      })
        .then(data => {
          this.meltList = data.list;
        })
        .catch(error => {
          console.log(error);
        });
    },
    handleFurnaceChange(val) {
      const { ribbonTypeName } =
        this.meltList.find(melt => melt.furnace === val) || {};
      if (ribbonTypeName) {
        this.formData.ribbonTypeName = ribbonTypeName;
      }
    },
    closeDialog() {
      this.$emit("close");
    },
    castTimesChangeHandler(val) {
      // 开包次数的改变
        ...
        if (valid) {
          ...
          const { method, url } =
            this.dialogData.formType === "add"
              ? { method: "POST", url: urlmap.addCast }
              : { method: "PUT", url: urlmap.updateCast };
