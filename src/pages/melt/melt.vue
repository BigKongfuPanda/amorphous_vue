<template>
  <div>
    ...
    <Collapse>
      <el-form class="search_bar" :model="searchForm" :inline="true">
        ...
        <el-form-item label="冶炼人：">
          <el-input
            v-model="searchForm.melter"
            placeholder="请输入冶炼人姓名"
          ></el-input>
        </el-form-item>
        ...
        <el-form-item label="桶号：">
          <el-input
            v-model="searchForm.bucket"
            placeholder="请输入桶号"
          ></el-input>
        </el-form-item>
        <el-form-item label="新料：">
          <el-input
            v-model="searchForm.newAlloyNumber"
            placeholder="请输入新料炉号"
          ></el-input>
        </el-form-item>
        <el-form-item label="加工料：">
          <el-input
            v-model="searchForm.oldAlloyNumber"
            placeholder="请输入加工料炉号"
          ></el-input>
        </el-form-item>
        <el-form-item>
            ...
            <el-button
              ...
              :disabled="
                userinfo.roleId !== 1 &&
                  userinfo.roleId !== 3 &&
                  userinfo.roleId !== 17 &&
                  userinfo.adminname !== scope.row.melter
              "
              >修改</el-button
            >
            <el-button
    ...
    <dialog-form
      v-if="dialogVisible"
      :dialogData="{ formType, dialogVisible, rowData, ribbonTypeList }"
      @close="closeHandler"
...
export default {
  ...
  methods: {
    ...
    setIsAble() {
      // 8：6#机组冶炼人员，10：7#机组冶炼人员，12：8#机组冶炼人员，14：9#机组冶炼人员，17：化钢组长
      if ([8, 10, 12, 14, 17].includes(this.userinfo.roleId)) {
        return true;
      } else {
        // 其他
        return false;
    ...
    getTableData(params = {}) {
      ...
      this.loading = true;
      this.$http("get", urlmap.queryMelt, params)
        .then(data => {
    ...
    del(row) {
      ...
      this.$confirm(`删除后数据无法恢复，确定删除 ${furnace} 吗？`, "提示", {
        ...
        .then(() => {
          this.$http("delete", urlmap.delMelt, { meltId })
            .then(data => {
              this.getTableData();
            })
            .catch(error => {
